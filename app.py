        ...
        try:
            ...
            for shipment in data.get('shipments', []):
                ...
                for item in shipment.get('orderItems', []):
                    # item.orderDate IS populated for preDispatch items
                    item_date = str(item.get('orderDate', '') or '')[:10]
                    if not item_date:
                        item_date = dispatch_after  # fallback to dispatchAfterDate
...
@app.route('/api/sales-backfill/<account>', methods=['POST'])
def sales_backfill(account):
    """
    Backfill sales data from a specific date.
    Body: { "from_date": "YYYY-MM-DD" }
    Runs in background thread like regular sync.
    """
    import threading
    account = account.upper().replace('-', ' ')
    if account not in SALES_ACCOUNTS:
    ...
    app_id, _ = _fk_credentials(account)
    if not app_id:
        return jsonify({'error': f'No API credentials for {account}. '
                                  f'Set {_fk_env_key(account)}_APP_ID and _APP_SECRET in Railway.'}), 400
    body = request.get_json() or {}
    from_date = body.get('from_date', '').strip()
    if not from_date:
    ...
    def _bg_backfill():
        try:
            ...
            s[sync_key] = True
            _save_sales_store(account, s)
            # Temporarily override the fetch window by patching the store
            # so _fk_sync_sales fetches from from_date regardless of existing data
            _fk_sync_sales_from(account, from_date)
            print(f'[Backfill] {account} from {from_date} completed')
        except Exception as e:
            print(f'[Backfill] {account} error: {e}')
            traceback.print_exc()
        finally:
            try:
                s = _load_sales_store(account)
                ...
                print(f'[Backfill] {account} flag clear failed: {fe}')

    t = threading.Thread(target=_bg_backfill, daemon=True)
    t.start()
    return jsonify({'ok': True, 'status': 'started',
                    'message': f'Backfill from {from_date} started for {account}. '
                               f'This may take several minutes depending on order volume.'})
...
# In-memory store for mark-rtd job status
_mark_rtd_status = {'running': False, 'result': None}

@app.route('/api/mark-rtd/status', methods=['GET'])
def mark_rtd_status():
    return jsonify(_mark_rtd_status)

@app.route('/api/mark-rtd', methods=['POST'])
def mark_rtd_trigger():
    """
    Fetch all PACKED shipments for selected accounts and mark them RTD.
    Runs in a background thread — returns immediately.
    Poll /api/mark-rtd/status for the result.
    """
    import requests as _req, threading as _threading
    data = request.get_json() or {}
    if data.get('pin') != '848424':
        return jsonify({'error': 'Invalid PIN'}), 403
    if _mark_rtd_status.get('running'):
        return jsonify({'error': 'Mark RTD already in progress'}), 409

    requested = data.get('accounts', list(FK_ACCOUNT_LOCATIONS.keys()))

    def _bg():
        import requests as _r
        _mark_rtd_status['running'] = True
        _mark_rtd_status['result']  = None
        results = {}
        ...
        for account in requested:
            ...
            # Fetch all PACKED shipments (= Pending RTD on portal)
            next_url = f'{base}/v3/shipments/filter/'
            payload  = {
                'filter': {
                    'type':       'preDispatch',
            ...
            try:
                while next_url and len(packed_ids) < 2000:
                    r = (_r.post(next_url, json=payload, headers=headers, timeout=30)
                         if next_url.endswith('/filter/')
                         else _r.get(next_url, headers=headers, timeout=30))
                    if r.status_code != 200:
                        break
                    resp_data = r.json()
            ...
            if not packed_ids:
                results[account] = {'ok': True, 'rtd': 0, 'found': 0, 'message': 'No PACKED shipments found'}
                print(f'[MarkRTD] {account}: no PACKED shipments')
                continue

            print(f'[MarkRTD] {account}: found {len(packed_ids)} PACKED shipments, marking RTD...')
        ...
        _mark_rtd_status['running'] = False
        _mark_rtd_status['result']  = {'ok': True, 'accounts': results, 'total_rtd': total_rtd}
        print(f'[MarkRTD] Complete: {total_rtd} total marked RTD')
...
def sales_sync_cron():
    """
    Called by Railway cron to sync sales for ALL configured accounts.
    Runs each account sync in parallel background threads.
    """
    import threading
    results = {}
    for account in SALES_ACCOUNTS:
        ...
        def _bg(acct=account):
            try:
                ...
                print(f'[SalesCron] {acct} error: {e}')
            finally:
                try:
                    s = _load_sales_store(acct)
                    s.pop(f'__syncing_{acct}__', None)
                    _save_sales_store(acct, s)
                except Exception:
                    pass

        threading.Thread(target=_bg, daemon=True).start()
        results[account] = 'started'
        print(f'[SalesCron] Started sync for {account}')
